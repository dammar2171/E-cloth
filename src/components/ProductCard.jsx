...
function ProductCard({ item }) {
  ...
  const onHandleDetail = () => {
    navigate(`/addcart/${item.type}/${item.id}`);
  };

  const DISCOUNT_PERCENTAGE = Math.round(
    ((item.mPrice - item.sPrice) / item.mPrice) * 100
  );

  return (
    <div className={`card ${style.costumCard}`} style={{ width: "20rem" }}>
      {" "}
      <span className={`${style.discountPer}`}>{DISCOUNT_PERCENTAGE}% OFF</span>
      <button
        type="button"
        onClick={() => {
